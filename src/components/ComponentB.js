...
const ComponentB = ({ onNextStep,chainLength,handleClose }) => {


    const handleFirst = () => {
        // Perform validation or other logic
        onNextStep(<ComponentG key={chainLength+1} onNextStep={onNextStep} chainLength={chainLength+1}/>); // Or determine the next component dynamically
    };

    const handleSecond = () => {
        // Perform validation or other logic
        onNextStep(<ComponentC key={chainLength+1} onNextStep={onNextStep} chainLength={chainLength+1}/>); // Or determine the next component dynamically
    };
        ...
        <div className="relative p-4 m-4 bg-white w-full rounded shadow sm:w-[600px]">
                <div className="flex-grow min-w-[64px] max-w-[600px]">
                    <div className="flex mb-4 w-full flex-nowrap items-center">
                        ...
                        <div className="font-bold w-full">Right, we need a model. Do you have a model already?</div>
                    </div>
