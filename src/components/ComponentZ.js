...
const ComponentZ = ({ onNextStep,chainLength,handleClose }) => {
    const [inputValue, setInputValue] = useState('');
    const [showWarning, setShowWarning] = useState(false);

    const handleSubmit = () => {
        // Perform validation or other logic
        onNextStep(<ComponentN key={chainLength+1} onNextStep={onNextStep} chainLength={chainLength+1}/>);
    };
        ...
        <div className="relative p-4 m-4 bg-white w-full rounded shadow sm:w-[600px]">
                <div className="flex-grow min-w-[64px] max-w-[600px]">
                    <div className="flex w-full flex-nowrap items-center">
                        ...
                        <div className="font-bold w-full">Alrigth, I got my head around this. What do you want to know?</div>
                    </div>
                    <textarea
                        value={inputValue}
